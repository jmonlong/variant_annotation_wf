...
if(all(ls() != 'sr')){
  load('references/annotation_database.RData')
}


gene.list = c(
  scan('genedx.anophthalmia.microphthalmia.txt', ''),
  scan('invitae.anophthalmia.microphthalmia.txt', ''),
  scan('preventiongenetics.anophthalmia.microphthalmia.txt', '')
) %>% unique
...
if(all(ls() != 'al.df')){
  al.df = loadVCF('./results/SG171796/SG171796.annot.vcf.gz')
}
...
```{r}
tr.samp = read.table('test.tsv.gz', header=TRUE, as.is=TRUE)
...
ol = findOverlaps(sr, tr.samp.gr, maxgap=1) %>% as.data.frame %>%
  mutate(size=tr.samp.gr$size[subjectHits],
         sample=tr.samp.gr$sample[subjectHits],
         phased=tr.samp.gr$phased[subjectHits],
         wol=width(pintersect(sr[queryHits], tr.samp.gr[subjectHits])),
         size=ifelse(size<0, -wol, size)) %>% 
  group_by(queryHits, sample, phased) %>% summarize(size=sum(size), .groups='drop')

sr.sv = sr[ol$queryHits]
sr.sv$size = ol$size
sr.sv$phased = ol$phased
sr.sv

tr.cont = read.table('references/hprc-hgsvc-tr-catalog.tsv.gz', as.is=TRUE, header=TRUE)
...
tr.df %>% filter(as.logical(phased), max_var<100) %>% arrange(desc(size-max_var)) %>%
  dplyr::select(-strand, -phased) %>% head(20)
## tr.df %>% filter(size>0) %>% 
##   ggplot(aes(x=max_var, y=size)) +
##   geom_point(alpha=.5) +
##   theme_bw() +
##   facet_wrap(~phased)
```
